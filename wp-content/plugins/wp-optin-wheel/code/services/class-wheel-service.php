...
namespace MABEL_WOF_LITE\Code\Services
{
	...
	class Wheel_service
	{
		...
		public static function calculate_segment_hit(Wheel_Model $wheel) {
			...
			if(!$will_win){
				$losing_segments = Enumerable::from($wheel->slices)->where('$x => $x->type == 0')->toArray();
				$losing_segment = $losing_segments[array_rand($losing_segments)];
				return $losing_segment;
			}else{
				$winning_segments = Enumerable::from($wheel->slices)->where('$x => $x->type != 0 && $x->chance != 0')->toArray();
